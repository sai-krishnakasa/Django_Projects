...
from Helmet_detect.settings import BASE_DIR
...
#                 new_bus=Bus.objects.create(bus_no=bus_no,bus_type=bus_type,seat_type=seat_type)
#                 new_bus.save()
#                 return redirect('add_bus')
#         return render(request,'add_new_bus.html')
...
#                 busss=Bus.objects.get(bus_no=request.POST['bus'])
...
#                         return render(request,'add_bus.html',{'drivers':drivers,'origin':origin,'destination':destination,'start_date':start_date})
#                 end_date=request.POST['end_time']
#                 if start_date>end_date:
#                         messages.error(request,f'Ending time Should Always be greater Than Starting time')
#                         return render(request,'add_bus.html',{'drivers':drivers,'origin':origin,'destination':destination,'start_date':start_date,'end_date':end_date})

#                 fare=request.POST['fare']
#                 driver_deta=driver_details.objects.filter(name=driver_name).first()
#                 driver_buses=driver_deta.buses.all()
#                 running_buses=Bus.objects.filter(bus_no=busss.bus_no).first()
#                 running_buses=running_buses.bus_details.all()
#                 print(running_buses)
#                 can_add=True
#                 can_add_bus=True
#                 buss=''
...
#                         if(can_add==False):
#                                 break
#                 for addbus in running_buses:
#                         if datetime.now(UTC)> addbus.start_time and datetime.now(UTC) < addbus.end_time:
#                                 can_add_bus=False
#                         if(can_add_bus==False):
#                                 break
                
#                 if(can_add_bus==False):
#                         messages.error(request,f'The Bus is Busy ')
#                         return redirect('add_bus')

#                 if (can_add==True and can_add_bus==True):
#                         bus_created=Buses.objects.create(driver_details=driver_deta, origin=origin,destination=destination,fare=fare,start_time=start_date,end_time=end_date,bus=busss)
#                         messages.success(request,f'Bus {busss.bus_no} Created Successfully')
#                         bus_created.save()
#                 else:
#                         messages.error(request,f'The Driver is Busy with bus {buss.bus.bus_no.upper()} {buss}  at this time ')
#                         return render(request,'add_bus.html',{'drivers':drivers,'origin':origin,'destination':destination,'start_date':start_date,'end_date':end_date,'fare':fare})
#                 return redirect('book')
#         origins=Buses.objects.values('origin').distinct()
#         dests=Buses.objects.values('destination').distinct()
#         bus_types=Buses.objects.values('bus__bus_type').distinct()
#         origins=[i['origin'] for i in origins ]
#         # drivers=[i['name'] for i in drivers ]
#         # print(drivers)
#         bus_types=[i['bus__bus_type'] for i in bus_types ]
#         buses=Bus.objects.all()
#         print(bus_types)
#         dests=[i['destination'] for i in dests ]
#         return render(request,'add_bus.html',{'origins':origins,'buses':buses,'bus_types':bus_types,'dests':dests,'drivers':drivers,'date':date})
...
#                 print(bus)
#                 bus.bus.not_booked_seats+=count
#                 bus.save()
